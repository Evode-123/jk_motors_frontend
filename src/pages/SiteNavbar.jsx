  ...
  return (
    <>
      <style>{`
        ...
        .snav-grad-btn  { background: linear-gradient(135deg,#C9A84C,#8B6914); }
      `}</style>
      ...
      >
        <div className="max-w-6xl mx-auto flex items-center justify-between h-[70px]">
          ...
          <Link to="/" className="flex items-center gap-2.5 no-underline">
            ...
            <div>
              ...
              <div className="snav-font-body text-[9px] tracking-[3px] uppercase leading-none" style={{ color: '#C9A84C' }}>Auto Parts & Service</div>
            </div>
          </Link>
          ...
          <div className="hidden md:flex gap-9">
            {navLinks.map(({ label, path }) => (
              <Link
                ...
                style={{ color: isActive(path) ? '#C9A84C' : 'rgba(148,163,184,0.8)' }}
                onMouseEnter={e => e.currentTarget.style.color = '#C9A84C'}
                onMouseLeave={e => e.currentTarget.style.color = isActive(path) ? '#C9A84C' : 'rgba(148,163,184,0.8)'}
              >
                {label}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              ...
              <>
                <Link to="/login"
                  className="snav-font-body text-sm font-semibold px-4 py-2 rounded-lg border transition-colors no-underline"
                ...
                <Link to="/signup"
                  ...
                  style={{ boxShadow: '0 4px 16px rgba(201,168,76,0.3)', color: '#fff' }}>
                  Get Started
                </Link>
              </>
            )}
          ...
          <button
            ...
            style={{ background: menuOpen ? 'rgba(201,168,76,0.1)' : 'transparent' }}
            aria-label="Toggle menu"
          >
            <div className={`w-5 h-0.5 rounded transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} style={{ background: '#C9A84C' }} />
            <div className={`w-5 h-0.5 rounded transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`} style={{ background: '#C9A84C' }} />
            <div className={`w-5 h-0.5 rounded transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`} style={{ background: '#C9A84C' }} />
          </button>
        </div>
        ...
        {/* Mobile menu */}
        <div
          className={`md:hidden transition-all duration-300 overflow-hidden relative z-50 ${menuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}
          style={{ background: 'rgba(8,8,8,0.98)', borderTop: '1px solid rgba(201,168,76,0.15)' }}
        >
          {navLinks.map(({ label, path }) => (
            <Link
              key={path}
              ...
              className="block w-full text-left px-6 py-4 snav-font-body text-sm font-semibold uppercase tracking-widest no-underline transition-colors"
              style={{ color: isActive(path) ? '#C9A84C' : 'rgba(148,163,184,0.8)', borderBottom: '1px solid rgba(201,168,76,0.08)' }}
            >
              {label}
            </Link>
          ))}
            ...
            {user ? (
              ...
              <>
                <Link to="/login" onClick={() => setMenuOpen(false)}
                  className="snav-font-body text-sm font-semibold py-3 rounded-xl border text-center no-underline transition-colors"
                  style={{ color: '#C9A84C', borderColor: 'rgba(201,168,76,0.3)', background: 'transparent' }}>
                  Sign In
                </Link>
