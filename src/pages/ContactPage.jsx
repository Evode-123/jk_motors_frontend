  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiService.submitContact({
        ...
        phone: form.phone || undefined, message: form.message,
      });
      setStatus('success');
  ...
  return (
    <div className="font-sans text-slate-100" style={{ background: '#0D0D0D', minHeight: '100vh' }}>
      ...
      <section className="relative pt-32 pb-20 px-6 grid-pattern overflow-hidden"
        style={{ background: 'radial-gradient(ellipse at 60% 40%, rgba(201,168,76,0.08) 0%, transparent 55%), #0A0A0A' }}>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="font-body text-xs font-semibold uppercase tracking-[3px]" style={{ color: '#C9A84C' }}>Get In Touch</span>
          </div>
      ...
      {/* ── CONTACT GRID ── */}
      <section className="py-18 px-6" style={{ background: '#080808' }}>
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10">
            ...
            {CONTACT_INFO.map(({ icon, label, value }) => (
              <div key={label} className="flex gap-3.5 items-start mb-5">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center text-lg flex-shrink-0"
                  style={{ background: 'rgba(201,168,76,0.06)', border: '1px solid rgba(201,168,76,0.15)' }}>{icon}</div>
                <div>
                  <div className="font-body text-[11px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: '#C9A84C' }}>{label}</div>
                  <div className="font-body text-sm text-slate-300">{value}</div>
                </div>
              </div>
            ))}
            <div className="mt-10 rounded-2xl p-5" style={{ background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.12)' }}>
          ...
          <div id="ct-f" data-animate
            ...
            <div className="relative rounded-2xl p-7 overflow-hidden"
              style={{ background: 'linear-gradient(135deg,#1C1509,#231B0B)', border: '1px solid rgba(201,168,76,0.18)' }}>
              ...
              {status === 'success' && (
                <div className="mb-5 flex items-center gap-3 px-4 py-3 rounded-xl"
                  ...
                  <p className="font-body text-sm text-emerald-400">Message sent! We'll be in touch shortly.</p>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <input type="tel" placeholder="Your Phone Number (optional)"
                  ...
                  style={inputBase} onFocus={onFocus} onBlur={onBlur} />
                <textarea placeholder="How can we help you?" required rows={4}
                ...
                <button type="submit" disabled={submitting}
                  ...
                  style={{ boxShadow: '0 4px 16px rgba(201,168,76,0.25)', opacity: submitting ? 0.7 : 1 }}>
                  {submitting ? (
                    <>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            <div className="flex items-center justify-center gap-4 mb-4">
              <div className="w-12 h-0.5 rounded-full" style={{ background: 'linear-gradient(90deg,#C9A84C,#8B6914)' }} />
              <span className="font-body text-xs font-semibold uppercase tracking-[3px]" style={{ color: '#C9A84C' }}>FAQ</span>
          ...
          <div className="space-y-3">
            {FAQ.map(({ q, a }, i) => (
              <div key={i} className="rounded-2xl overflow-hidden"
                ...
                {openFaq === i && (
                  <div className="px-6 pb-4" style={{ borderTop: '1px solid rgba(201,168,76,0.1)' }}>
